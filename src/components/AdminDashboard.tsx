...
export const AdminDashboard: React.FC<Props> = ({
  ...
  const [statusFilter, setStatusFilter] = useState<'Todos' | 'Activo' | 'Pendiente' | 'Finalizado'>('Todos');
  ...
  const pendingCount = volunteers.filter((v) => v.status === 'Pendiente').length;
  const activeCount = volunteers.filter((v) => v.status === 'Activo').length;
  const finishedCount = volunteers.filter((v) => v.status === 'Finalizado').length;
  ...
  const handleEnableVolunteer = async (vol: Volunteer) => {
    const updated: Volunteer = {
      ...vol,
      status: 'Activo',
      endDate: vol.endDate?.includes('habilitación') ? 'Activo actualmente' : vol.endDate,
    };

    onUpdateVolunteers(volunteers.map((v) => (v.id === vol.id ? updated : v)));

    try {
      await saveVolunteerToFirebase(updated);
      showToast(`¡Colaborador ${vol.fullName} habilitado exitosamente! Ya tiene acceso para iniciar sesión con su cédula.`);
    } catch (err) {
      console.error('Error al guardar en Firebase:', err);
      showToast(`Habilitado localmente: ${vol.fullName}`);
    }
  };
  ...
  const filteredVolunteers = volunteers.filter((vol) => {
    if (statusFilter !== 'Todos' && vol.status !== statusFilter) {
      return false;
    }
  ...
  return (
    <div className="min-h-screen pb-16">
      ...
      <header className="bg-white/75 backdrop-blur-xl border-b border-sky-200/70 sticky top-0 z-20 shadow-xs shadow-blue-900/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 min-h-24 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2.5 sm:gap-3">
            ...
            {/* Habilitar / Pausar Registro Público en el Login */}
            <button
              type="button"
              onClick={() => {
                const newState = !registrationEnabled;
                onToggleRegistration?.(newState);
                showToast(
                  newState
                    ? 'El botón de registro en el inicio de sesión está ahora HABILITADO.'
                    : 'El botón de registro en el inicio de sesión ha sido PAUSADO.'
                );
              }}
              ...
              title={
                registrationEnabled
                  ? 'El botón de registro público en el login está habilitado. Haz clic para pausarlo.'
                  : 'El botón de registro público en el login está pausado. Haz clic para habilitarlo.'
              }
            >
              <UserPlus className="w-4 h-4 text-blue-700" />
              <span className="hidden sm:inline">Registro Login:</span>
              <span className={registrationEnabled ? 'text-emerald-700 font-extrabold' : 'text-slate-500 font-semibold'}>
                {registrationEnabled ? 'Habilitado' : 'Pausado'}
              </span>
            </button>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-6">
        {/* Banner de Solicitudes Pendientes de Habilitación */}
        {pendingCount > 0 && (
          <div className="p-4 sm:p-5 rounded-3xl bg-gradient-to-r from-amber-500/15 via-amber-100/90 to-orange-50 border-2 border-amber-300/90 shadow-md shadow-amber-950/5 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-start sm:items-center gap-3.5">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="px-2.5 py-0.5 rounded-full text-xs font-extrabold bg-amber-200 text-amber-900 border border-amber-300 uppercase tracking-wide">
                    {pendingCount} {pendingCount === 1 ? 'Solicitud Pendiente' : 'Solicitudes Pendientes'}
                  </span>
                  <span className="text-xs font-semibold text-amber-900">
                    Requiere que el Administrador la habilite
                  </span>
                </div>
                <p className="text-sm font-bold text-slate-900 mt-1">
                  Nuevos colaboradores se han registrado desde la pantalla de inicio y esperan tu autorización.
                </p>
                <p className="text-xs text-slate-600">
                  Haz clic en &ldquo;Habilitar&rdquo; en la fila del colaborador para activar inmediatamente su ingreso con cédula.
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className={`px-4 py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all shadow-sm cursor-pointer ${
              ...
              {statusFilter === 'Pendiente' && (
                <button
                  ...
                  className="px-3.5 py-2.5 bg-white hover:bg-slate-50 text-slate-700 border border-slate-300 rounded-xl font-semibold text-xs cursor-pointer"
                >
                  Ver Todos
                </button>
              )}
        ...
        <div className="bg-white/85 backdrop-blur-md rounded-3xl border border-sky-200/70 shadow-md shadow-blue-950/5 overflow-hidden">
          ...
          {/* Status Filter Tabs */}
          <div className="px-5 py-2.5 bg-sky-50/40 border-b border-sky-100 flex flex-wrap items-center gap-2 text-xs font-semibold">
            <span className="text-slate-500 mr-1">Filtrar por estado:</span>
            <button
              type="button"
              onClick={() => setStatusFilter('Todos')}
                  ...
                  filteredVolunteers.map((vol) => (
                    <tr key={vol.id} className="hover:bg-blue-50/40 transition-colors">
                      ...
                      <td className="py-4 px-4">
                        ...
                        >
                          {vol.status === 'Pendiente' && <Clock className="w-3 h-3 text-amber-600 shrink-0" />}
                          {vol.status === 'Pendiente' ? 'Por Habilitar' : vol.status}
                        </span>
                      </td>
                      <td className="py-4 px-5 text-right">
                        <div className="flex items-center justify-end gap-1.5 flex-wrap">
                          {vol.status === 'Pendiente' && (
                            <button
                              type="button"
                              onClick={() => handleEnableVolunteer(vol)}
                              title={`Habilitar inmediatamente a ${vol.fullName} para que pueda iniciar sesión`}
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white rounded-xl text-xs font-bold transition-all shadow-sm shadow-emerald-600/20 cursor-pointer hover:scale-105"
                            >
